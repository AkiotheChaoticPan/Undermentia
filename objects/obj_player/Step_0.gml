...
// stops player from moving when menu is open or when the player is in a dialogue.
if (global.menu_open) {
	is_moving = false;
	return;
...
if(can_interact()) {
	//Sets the direction of interactable trigger detection
	var _x_offset = _move_x * collided_length;
	var _y_offset = _x_offset != 0 ? 0 : _move_y * collided_length;

	//Saves the direction when the player is not moving
